...
type Company = { user_id: string; email:string; name:string; jib:string; city:string; plan:string; paid_until:string|null; price_bam:number; created_at:string };
export function AdminTable() {
  ...
  const save = async (c:Company) => {
    ...
    try { const r = await fetch("/api/admin",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({userId:c.user_id,priceBam:Number(c.price_bam)})}); const d=await r.json() as {error?:string}; if(!r.ok) throw new Error(d.error); await load(); }
