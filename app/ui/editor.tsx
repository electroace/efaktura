...
export function Editor({isFree,used,vatRegistered,company,customers,catalogItems,initial}:{isFree:boolean;used:number;vatRegistered:boolean;company:Company;customers:Customer[];catalogItems:CatalogItem[];initial?:Doc}) {
  const [doc,setDoc] = useState<Doc>(initial ?? {type:"invoice",title:company.invoice_title,number:"",issueDate:today(),dueDate:"",clientName:"",clientAddress:"",clientId:"",clientContact:"",showClientContact:false,showIssuerContact:false,fiscalNumber:"",currency:"KM",items:[{name:"",quantity:1,unit:"kom",price:0,vat:vatRegistered?17:0,discount:0}],notes:company.default_note});
  const [busy,setBusy] = useState(false);
  const [dirty,setDirty] = useState(false);
  const [error,setError] = useState("");
  const [notice,setNotice] = useState("");
  const issuer:Issuer = doc.issuer ? {...doc.issuer,contactPerson:doc.issuer.contactPerson ?? company.contact_person} : {name:company.name,address:company.address,city:company.city,postalCode:company.postal_code,jib:company.jib,vatId:company.vat_id,vatRegistered:!!company.vat_registered,iban:company.iban,bank:company.bank,phone:company.phone,email:company.contact_email,contactPerson:company.contact_person,responsiblePerson:company.responsible_person,electronicNotice:!!company.electronic_notice,signatureLine:!!company.show_signature_line,showDiscount:!!company.show_discount,logoKey:company.logo_key};
  ...
  return <main className="editor-layout">
    <form className="editor-panel" onSubmit={save}>
      ...
      <div className="form-grid">
        <label>Vrsta dokumenta <Select value={doc.type} disabled={!!initial?.id} onValueChange={v=>{setDoc(prev=>({...prev,type:v,title:v==="invoice"?company.invoice_title:v==="offer"?company.offer_title:prev.type==="custom"?prev.title:""}));setDirty(true);}}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="invoice">{company.invoice_title}</SelectItem><SelectItem value="offer">{company.offer_title}</SelectItem><SelectItem value="custom">Drugi dokument</SelectItem></SelectContent></Select></label>
      ...
      <div className="item-stack">{doc.items.map((item,index)=><div className="item-card" key={index}>
        ...
        <div className="item-fields"><label>Količina <Input type="number" min="0.001" step="any" required value={item.quantity} onChange={e=>setItem(index,"quantity",Number(e.target.value))}/></label>
          ...
          <label>PDV % <Input type="number" min="0" max="100" step="0.01" disabled={!issuer.vatRegistered} value={issuer.vatRegistered?item.vat:0} onChange={e=>setItem(index,"vat",Number(e.target.value))}/></label>
          {issuer.showDiscount&&<label>Rabat % <Input type="number" min="0" max="100" step="0.01" value={item.discount??0} onChange={e=>setItem(index,"discount",Number(e.target.value))}/></label>}</div>
      </div>)}</div>
      <Button type="button" variant="outline" disabled={doc.items.length >= (isFree?5:100)} onClick={()=>set("items",[...doc.items,{name:"",quantity:1,unit:"kom",price:0,vat:issuer.vatRegistered?17:0,discount:0}])}><Plus size={16}/> Dodaj stavku</Button>
      ...
      <div className="print-options"><strong>Prikaži kontakt osobe na PDF-u i Word dokumentu</strong>
        <label><input type="checkbox" checked={doc.showIssuerContact} onChange={e=>set("showIssuerContact",e.target.checked)}/> Kontakt osoba moje firme {issuer.contactPerson?`(${issuer.contactPerson})`:"(unesite je u Postavkama)"}</label>
    ...
    <aside className="preview-column"><div className="preview-label"><span>PREGLED DOKUMENTA</span><span>A4</span></div><article className="document-paper" id="document-paper">
      <div className="paper-topline"><div className="paper-brand">{issuer.logoKey?<img className="paper-logo" src={doc.id ? `/api/logo?document=${encodeURIComponent(doc.id)}` : "/api/logo"} alt="Logo firme"/>:<span className="paper-monogram">{issuer.name.slice(0,1).toLocaleUpperCase("bs")}</span>}</div><div className="paper-issuer"><strong>{issuer.name}</strong><p>{issuer.address}<br/>{issuer.postalCode} {issuer.city}<br/>JIB: {issuer.jib}{issuer.vatId&&<> · PDV: {issuer.vatId}</>}<br/>{issuer.phone}{issuer.phone&&issuer.email?" · ":""}{issuer.email}{doc.showIssuerContact&&issuer.contactPerson&&<><br/>Kontakt osoba: {issuer.contactPerson}</>}</p></div></div>
      <div className="paper-header"><div className="paper-title"><small>POSLOVNI DOKUMENT</small><h2>{doc.title || "Dokument"}</h2><strong>{doc.number || "Broj po spremanju"}</strong></div></div>
      <div className="paper-parties"><div><span>IZDATO ZA</span><strong>{doc.clientName || "Naziv kupca"}</strong><p>{doc.clientAddress}{doc.clientAddress&&doc.clientId?<br/>:null}{doc.clientId&&`JIB: ${doc.clientId}`}{doc.showClientContact&&doc.clientContact&&<><br/>Kontakt osoba: {doc.clientContact}</>}</p></div><div className="paper-facts"><div><span>DATUM IZDAVANJA</span><strong>{date(doc.issueDate)}</strong></div>{doc.dueDate&&<div><span>{doc.type==="offer"?"PONUDA VAŽI DO":"ROK PLAĆANJA"}</span><strong>{date(doc.dueDate)}</strong></div>}{doc.fiscalNumber&&<div><span>BF</span><strong>{doc.fiscalNumber}</strong></div>}</div></div>
      <table className="paper-table"><thead><tr><th>Opis</th><th>Kol.</th><th>Cijena</th>{issuer.showDiscount&&<th>Rabat</th>}{issuer.vatRegistered&&<th>PDV</th>}<th>Iznos</th></tr></thead><tbody>{doc.items.map((i,n)=><tr key={n}><td><strong>{i.name||"Nova stavka"}</strong>{i.sku&&<small>Šifra: {i.sku}</small>}{i.description&&<small>{i.description}</small>}<small>{i.unit}</small></td><td>{i.quantity}</td><td>{money(i.price,doc.currency)}</td>{issuer.showDiscount&&<td>{i.discount??0}%</td>}{issuer.vatRegistered&&<td>{i.vat}%</td>}<td>{money(lineAmounts(i).net/100,doc.currency)}</td></tr>)}</tbody></table>
      <div className="paper-payment-row"><div className="paper-payment">{(issuer.iban||issuer.bank)&&<><span>ZA UPLATU</span><strong>{issuer.bank}</strong><p>{issuer.iban}</p></>}</div><div className="invoice-total">{issuer.showDiscount&&<><div><span>Prije rabata</span><strong>{money(totals.gross/100,doc.currency)}</strong></div><div><span>Rabat</span><strong>−{money(totals.discount/100,doc.currency)}</strong></div></>}<div><span>Osnovica</span><strong>{money(totals.net/100,doc.currency)}</strong></div>{issuer.vatRegistered&&<div><span>PDV</span><strong>{money(totals.vat/100,doc.currency)}</strong></div>}<div className="grand-total"><span>Ukupno za plaćanje</span><strong>{money((totals.net+totals.vat)/100,doc.currency)}</strong></div></div></div>
      {doc.notes&&<div className="paper-note"><span>NAPOMENA</span><p>{doc.notes}</p></div>}
      <footer className="paper-bottom">{(issuer.responsiblePerson||issuer.signatureLine)&&<div className="paper-signature">{issuer.signatureLine&&<div className="paper-signature-rule"/>}<strong>{issuer.responsiblePerson||"Odgovorno lice"}</strong><span>ODGOVORNO LICE{issuer.signatureLine?" · PEČAT I POTPIS":""}</span></div>}{issuer.electronicNotice&&<p>Dokument je elektronski izdat i važi bez pečata i potpisa.</p>}</footer>
