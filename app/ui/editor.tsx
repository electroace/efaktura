...
import type { Company } from "@/lib/server";
...
type Doc = {id?:string;type:string;title:string;number:string;issueDate:string;dueDate:string;clientName:string;clientAddress:string;clientId:string;currency:string;items:Item[];notes:string;issuer?:Issuer};
...
export function Editor({isFree,used,vatRegistered,company,initial}:{isFree:boolean;used:number;vatRegistered:boolean;company:Company;initial?:Doc}) {
  ...
  const issuer:Issuer = doc.issuer ?? {name:company.name,address:company.address,city:company.city,postalCode:company.postal_code,jib:company.jib,vatId:company.vat_id,vatRegistered:!!company.vat_registered,iban:company.iban,bank:company.bank,phone:company.phone,email:company.contact_email,logoKey:company.logo_key};
  ...
  return <main className="editor-layout">
    <form className="editor-panel" onSubmit={save}>
      ...
      <div className="form-grid"><label className="full">Naziv kupca <Input required value={doc.clientName} onChange={e=>set("clientName",e.target.value)} placeholder="Naziv firme ili ime i prezime"/></label>
    ...
    <aside className="preview-column"><div className="preview-label"><span>PREGLED DOKUMENTA</span><span>A4</span></div><article className="document-paper" id="document-paper">
      <div className="paper-header"><div>{issuer.logoKey && <img className="paper-logo" src={doc.id ? `/api/logo?document=${encodeURIComponent(doc.id)}` : "/api/logo"} alt="Logo firme"/>}<h2>{issuer.name}</h2><p>{issuer.address}<br/>{issuer.postalCode} {issuer.city}<br/>JIB: {issuer.jib}{issuer.vatId && <> · PDV: {issuer.vatId}</>}<br/>{issuer.phone}{issuer.phone&&issuer.email?" · ":""}{issuer.email}</p></div><div className="paper-title"><span>{doc.title || "Dokument"}</span><strong>{doc.number || "Broj po spremanju"}</strong></div></div>
      <div className="paper-parties"><div><span>IZDANO ZA</span><strong>{doc.clientName || "Naziv kupca"}</strong><p>{doc.clientAddress}{doc.clientAddress&&doc.clientId?<br/>:null}{doc.clientId&&`JIB: ${doc.clientId}`}</p></div><div><span>DETALJI</span><p>Datum izdavanja: <strong>{date(doc.issueDate)}</strong></p>{doc.dueDate&&<p>{doc.type==="offer"?"Važi do":"Rok plaćanja"}: <strong>{date(doc.dueDate)}</strong></p>}</div></div>
      <table className="paper-table"><thead><tr><th>Opis</th><th>Kol.</th><th>Cijena</th>{issuer.vatRegistered&&<th>PDV</th>}<th>Iznos</th></tr></thead><tbody>{doc.items.map((i,n)=><tr key={n}><td>{i.name||"Nova stavka"}<small>{i.unit}</small></td><td>{i.quantity}</td><td>{money(i.price,doc.currency)}</td>{issuer.vatRegistered&&<td>{i.vat}%</td>}<td>{money(Math.round((Number(i.quantity)||0)*(Number(i.price)||0)*100)/100,doc.currency)}</td></tr>)}</tbody></table>
