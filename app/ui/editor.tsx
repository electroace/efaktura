...
export function Editor({isFree,used,vatRegistered,company,customers,catalogItems,initial}:{isFree:boolean;used:number;vatRegistered:boolean;company:Company;customers:Customer[];catalogItems:CatalogItem[];initial?:Doc}) {
  ...
  return <main className="editor-layout">
    ...
    <aside className="preview-column"><div className="preview-label"><span>PREGLED DOKUMENTA</span><span>A4</span></div><article className="document-paper" id="document-paper">
      <div className="paper-topline"><div className="paper-brand">{issuer.logoKey?<img className="paper-logo" src={doc.id ? `/api/logo?document=${encodeURIComponent(doc.id)}` : "/api/logo"} alt="Logo firme"/>:<span className="paper-monogram">{issuer.name.slice(0,1).toLocaleUpperCase("bs")}</span>}<strong>{issuer.name}</strong></div><span>POSLOVNI DOKUMENT</span></div>
      <div className="paper-header"><div className="paper-title"><small>{doc.type==="invoice"?"FAKTURA":doc.type==="offer"?"PONUDA":"DOKUMENT"}</small><h2>{doc.title || "Dokument"}</h2><strong>{doc.number || "Broj po spremanju"}</strong></div><div className="paper-issuer"><span>IZDAVALAC</span><strong>{issuer.name}</strong><p>{issuer.address}<br/>{issuer.postalCode} {issuer.city}<br/>JIB: {issuer.jib}{issuer.vatId&&<> · PDV: {issuer.vatId}</>}<br/>{issuer.phone}{issuer.phone&&issuer.email?" · ":""}{issuer.email}{doc.showIssuerContact&&issuer.contactPerson&&<><br/>Kontakt osoba: {issuer.contactPerson}</>}</p></div></div>
      ...
      <table className="paper-table"><thead><tr><th>Opis</th><th>Kol.</th><th>Cijena</th>{issuer.vatRegistered&&<th>PDV</th>}<th>Iznos</th></tr></thead><tbody>{doc.items.map((i,n)=><tr key={n}><td><strong>{i.name||"Nova stavka"}</strong>{i.sku&&<small>Šifra: {i.sku}</small>}{i.description&&<small>{i.description}</small>}<small>{i.unit}</small></td><td>{i.quantity}</td><td>{money(i.price,doc.currency)}</td>{issuer.vatRegistered&&<td>{i.vat}%</td>}<td>{money(Math.round((Number(i.quantity)||0)*(Number(i.price)||0)*100)/100,doc.currency)}</td></tr>)}</tbody></table>
      <div className="paper-payment-row"><div className="paper-payment">{(issuer.iban||issuer.bank)&&<><span>ZA UPLATU</span><strong>{issuer.bank}</strong><p>{issuer.iban}</p></>}</div><div className="invoice-total"><div><span>Osnovica</span><strong>{money(subtotal,doc.currency)}</strong></div>{issuer.vatRegistered&&<div><span>PDV</span><strong>{money(vat,doc.currency)}</strong></div>}<div className="grand-total"><span>Ukupno za plaćanje</span><strong>{money(subtotal+vat,doc.currency)}</strong></div></div></div>
