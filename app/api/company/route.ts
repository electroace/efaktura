...
export async function POST(request: Request) {
  ...
  try {
    ...
    const contactPerson = clean(data.contactPerson, 120);
    if (!existing && (!phone || !contactPerson)) return apiError("Unesite ime i prezime kontakt osobe i telefon.");
    const invoiceTitle = data.invoiceTitle === "Račun" ? "Račun" : "Faktura";
    const offerTitle = data.offerTitle === "Predračun" ? "Predračun" : "Ponuda";
    ...
    const values = [
      ...
      phone, clean(data.contactEmail, 120) || user.email,
      contactPerson, clean(data.defaultNote, 1600),
      ...
      invoiceTitle, offerTitle, data.showDiscount === true ? 1 : 0,
      now, user.id
    ];
    if (existing) {
      await db().prepare(`UPDATE companies SET name=?, address=?, city=?, postal_code=?, jib=?, vat_id=?,
        ...
        responsible_person=?, electronic_notice=?, show_signature_line=?, invoice_title=?, offer_title=?,
        show_discount=?, updated_at=? WHERE user_id=?`).bind(...values).run();
    } else {
      await db().prepare(`INSERT INTO companies
        (name,address,city,postal_code,jib,vat_id,vat_registered,iban,bank,phone,contact_email,contact_person,
         default_note,responsible_person,electronic_notice,show_signature_line,invoice_title,offer_title,show_discount,
         updated_at,user_id,email,created_at)
