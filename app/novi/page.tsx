...
import { companyFor, currentMonth, db } from "@/lib/server";
...
export default async function NewPage() {
  ...
  const usage = await db().prepare("SELECT COUNT(*) AS count FROM documents WHERE user_id=? AND month=?").bind(user.userId,currentMonth()).first<{count:number}>();
  return <div className="app-frame"><header className="app-header"><Link href="/" className="brand"><span className="brand-mark">e</span>faktura<span className="brand-dot">.</span>ba</Link><Link href="/">← Svi dokumenti</Link></header>
    <Editor isFree={company.plan === "free"} used={usage?.count ?? 0} vatRegistered={!!company.vat_registered} company={company}/></div>;
}
