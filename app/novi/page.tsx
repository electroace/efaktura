...
export default async function NewPage() {
  ...
  return <div className="app-frame"><AppHeader user={user} hasCompany paid={paidActive(company)}/>
    <Editor isFree={!paidActive(company)} used={usage?.count ?? 0} vatRegistered={!!company.vat_registered} company={company} customers={customers.results} catalogItems={items.results}/></div>;
}
